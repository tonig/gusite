	...
	public StringBuffer getHtmlEncuesta(HttpServletRequest request, Long idmicrosite, String idencuesta, String idioma) {
		...
		try { 			
			...
			if ( (encuesta.getVisible().equals("S")) && (Fechas.vigente(encuesta.getFpublicacion(), encuesta.getFcaducidad())) ){
			    ...
			    while (iter.hasNext()) { // Preguntas de una encuesta
	    			...
	    			if ( (pregunta.getVisible().equals("S")) && (pregunta.getVisiblecmp().equals("S")) ) {
		    			...
		    			if ("S".equals(pregunta.getMultiresp())){		    									    			    								    				
		    				if (min!=0 && max!=0){
		    					//obligatoriedad =  new String (rb.getSting("clau").getBytes("ISO-8859-1"),"UTF-8");
		    					...
		    					scriptValidar.append("if (comptaChecks(\"C" + pregunta.getId() +"\") > " + max + " || comptaChecks(\"C" + pregunta.getId() +"\") < " + min + ") ");
					    ...
					    while (iter2.hasNext()) {
					    	...
					    	if (pregunta.getMultiresp().equals("S")) {
					    		...
					    		retorno.append("<input type=\"checkbox\" id=\"" + compId + "\" name=\"C" + pregunta.getId() + "\" value=\"" + respuesta.getId() + "\"" + disabledChk + checked + ">");
					    	} else {
					    		scriptRadioDisabled += "document.getElementById(\"" + compId + "\").disabled=true;\n";
					    		
					    		retorno.append("<input type=\"radio\"id=\"" + compId + "\"name=\"R" + pregunta.getId() + "\" value=\"" + respuesta.getId() + "\"" + checked + ">");					    		
					    	}
					    	...
					    	if (respuesta.getTipo().equals("I")) {
					    		...
					    		//retorno.append("<input type=\"text\" name=\"T"  + respuesta.getId() + "_" + pregunta.getId() + "\"" + readonly + ">");
					    		String onKeyUp = "onKeyUp=\"marcaCheck('" + respuesta.getId() + "','" + pregunta.getId() + "');\"";					    		
					    		retorno.append("<br><TEXTAREA COLS=\"120\" ROWS=\"2\" " + onKeyUp + " name=\"T"  + respuesta.getId() + "_" + pregunta.getId() + "\"" + disabledTxt + "></TEXTAREA>");
					    		if (param.get(respuesta.getId()) != null){ 
					    			retorno.append("<script>document.getElementById(\"encuesta\").T" + respuesta.getId() + "_" + pregunta.getId() + ".value='" + (String)param.get(respuesta.getId()) + "';</script>");					    			
					    		}
					    	}
					    	retorno.append("</li>\n");				    	
					    }
			    ...
			    scriptValidar.append("; \n if (txtError == \"\") { \n document.encuesta.submit(); \n } else { \n alert(\"" + rb.getString("encuesta.condiciones") + "\" + txtError); } }\n</script>\n");

			    retorno.append("<div id=\"botoneraCom\"><label><input type=\"button\" onClick = \"validaencuesta();\" name=\"btnanar\" value=\"" + rb.getString("encuesta.enviar") + "\" /></label></div>\n");
			    retorno.append("</form>\n");
			    if (encuesta.getMostrar().equalsIgnoreCase("S")){
			    	retorno.append("<p style=\"text-align:center;\"><a href=\"envioencuesta.do?" + Microfront.PIDSITE + "=" + idmicrosite + "&" + Microfront.PCONT + "=" + idencuesta + "&" + Microfront.PLANG + "=" + idioma + "&" + Microfront.PVIEW + "=yes\">" + rb.getString("encuesta.verresultados") + "</a></p>\n");
				}
				...
				retorno.append("<div id=\"enquestaPreguntaCom\">\n");
	    		retorno.append("<h3>" + ((encuesta.getTraduce()!=null)?((TraduccionEncuesta)encuesta.getTraduce()).getTitulo():"&nbsp;") + "</h3>\n");
